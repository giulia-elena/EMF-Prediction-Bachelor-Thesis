...
scaler = joblib.load(os.path.join(DIR_MODELE, 'scaler_cnn_keras.joblib')) # Foloseșc același scaler dacă e compatibil
model = load_model(os.path.join(DIR_MODELE, 'model_ann_keras_referinta.h5'), compile=False)

print("\nÎncepe analiza SHAP pentru ANN")

# 4. Wrapper pentru scalare
def predict_ann_wrapper(x):
    x_scaled = scaler.transform(x)
    return model.predict(x_scaled, verbose=0).flatten()

# 5. Configurare SHAP (KernelExplainer)
explainer = shap.KernelExplainer(predict_ann_wrapper, x_train_background)
...
plt.title('Impactul Caracteristicilor - Model ANN Referință', fontsize=14, pad=20)
plt.tight_layout()

nume_fisier_grafic = 'Grafic_SHAP_ANN_Referinta.png'
